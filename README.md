...
Главная страница, каталог и корзина тестируются с помощью npm run test:unit
Для проверки конкретного бага можно поменять BUG_ID в скрипте test:unit
Тесты e2e на hermione не дописаны. Работает только jest тестирование

Для связи пишите в телеграме @geek_py
